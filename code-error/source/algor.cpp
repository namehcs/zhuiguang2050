...
bool Result::Forward_Backward(Data& data, int& bestline) {
	bool result = true;
...
void Result::Algorithm(Data& data) {
	int bestline = 0;
	bool forward_result;
	forward_result = Forward_Backward(data, bestline);
	if (forward_result) {
		/*拆环顺序：
		1、比较自回环最大次数和第一类环回范围
...
bool Result::Check_Match_back(Data& data, int dev_index, int wind, int wind_index, int line, int& area_index) {
	...
	for (auto& next_dev : data.device_data[dev_index].next_device) {
		if (data.linegraph.adjacent_matrix[dev_index][next_dev->index] != 2 &&  //不是协同设备
